...
public class Roles {
    ...
    public synchronized static List<Roles> comprobandoRol(String userName) {
        /*
            La siguiente consulta la tabla es de uso public
        */
    ...
    public synchronized static List<Roles> getPermisosRoles(String rol){
        final String sql = "SELECT USER_NAME, NOMBRE_RELACION, DESCRIPCION " +
                "FROM GET_PRIVILEGIOS " +
                "WHERE TRIM(USER_NAME) = ? AND NOMBRE_RELACION STARTING 'SP_'";
        List<Roles> roles = new ArrayList<>();
        try (PreparedStatement ps = getCnn().prepareStatement(
                sql,
            ...
            try (ResultSet rs = ps.executeQuery();) {
                while (rs.next()) {
                    roles.add(Roles.builder().
                            roleName(rs.getString("USER_NAME")).
                            descripcion(rs.getString("DESCRIPCION")).
                            nombreProcedimiento(rs.getString("NOMBRE_RELACION")).build());
                }
            } 
        }catch (SQLException ex) {
            LOG.log(Level.SEVERE, ex.getMessage(), ex);
