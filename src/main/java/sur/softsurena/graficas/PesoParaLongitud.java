...
public class PesoParaLongitud {
        ...
        try {
            ...
            for (str = localBufferedReader.readLine();
                ...
                if (str.substring(8, 12).trim().equals("m") ) {//Determina Sexo
                    ...
                    if (f1 == 110.0) {
                        SD3Neg = Float.parseFloat(str.substring(53, 59).trim())  ;
                        SD2Neg = Float.parseFloat(str.substring(64, 70).trim())  ;
                        SD1Neg = Float.parseFloat(str.substring(75, 81).trim())  ;
                        SD0 =    Float.parseFloat(str.substring(86, 92).trim())  ;
                        SD1 =    Float.parseFloat(str.substring(95, 101).trim()) ;
                        SD2 =    Float.parseFloat(str.substring(104, 110).trim());
                        SD3 =    Float.parseFloat(str.substring(112, 117).trim());
                    }
                }
    ...
    private JFreeChart createChart(XYDataset paramXYDataset) {
        ...
        TextTitle localTextTitle1 = new TextTitle("Peso para la Longitud",
                new Font("SansSerif", 1, 14));

        TextTitle localTextTitle2 = new TextTitle(
                "Nacimiento a 2 años, Puntuación",
        ...
        XYTextAnnotation localXYTextAnnotation = new XYTextAnnotation("-3", 110, SD3Neg);
        ...
        localXYTextAnnotation = new XYTextAnnotation("-2", 110, SD2Neg);
        localXYTextAnnotation.setFont(localFont);
        localXYTextAnnotation.setTextAnchor(TextAnchor.HALF_ASCENT_LEFT);
        localXYPlot.addAnnotation(localXYTextAnnotation);

        localXYTextAnnotation = new XYTextAnnotation("-1", 110, SD1Neg);
        localXYTextAnnotation.setFont(localFont);
        localXYTextAnnotation.setTextAnchor(TextAnchor.HALF_ASCENT_LEFT);
        localXYPlot.addAnnotation(localXYTextAnnotation);

        localXYTextAnnotation = new XYTextAnnotation("0", 110, SD0);
        localXYTextAnnotation.setFont(localFont);
        localXYTextAnnotation.setTextAnchor(TextAnchor.HALF_ASCENT_LEFT);
        localXYPlot.addAnnotation(localXYTextAnnotation);

        localXYTextAnnotation = new XYTextAnnotation("1", 110, SD1);
        localXYTextAnnotation.setFont(localFont);
        localXYTextAnnotation.setTextAnchor(TextAnchor.HALF_ASCENT_LEFT);
        localXYPlot.addAnnotation(localXYTextAnnotation);

        localXYTextAnnotation = new XYTextAnnotation("2", 110, SD2);
        localXYTextAnnotation.setFont(localFont);
        localXYTextAnnotation.setTextAnchor(TextAnchor.HALF_ASCENT_LEFT);
        localXYPlot.addAnnotation(localXYTextAnnotation);

        localXYTextAnnotation = new XYTextAnnotation("3", 110, SD3);
        localXYTextAnnotation.setFont(localFont);
        localXYTextAnnotation.setTextAnchor(TextAnchor.HALF_ASCENT_LEFT);
        localXYPlot.addAnnotation(localXYTextAnnotation);
