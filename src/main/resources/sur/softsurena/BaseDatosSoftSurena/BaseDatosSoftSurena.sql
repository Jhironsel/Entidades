...
CREATE SEQUENCE GEN_ID_ASEGURADOS START WITH 1 INCREMENT BY 1;
CREATE SEQUENCE G_ID_ARS ;
CREATE SEQUENCE G_ID_CATEGORIAS ;
CREATE SEQUENCE G_ID_CONTACTOS ;
CREATE SEQUENCE G_ID_FACTURA ;
CREATE SEQUENCE G_ID_PERSONA ;
CREATE SEQUENCE G_ID_PRODUCTOS ;
/******************** DOMAINS *********************/
...
CREATE DOMAIN D_SEXO
 AS CHAR(1)
 DEFAULT 'M'
 CHECK (UPPER(value) = 'M' or UPPER(value) = 'F')
 COLLATE ISO8859_1;
CREATE DOMAIN D_TELEFONO
 AS VARCHAR(14)
 CHECK (value similar to '[(][[:digit:]]{3}[)][ ][[:digit:]]{3}[-][[:digit:]]{4}')
 COLLATE ISO8859_1;
...
(
  ID D_ID NOT NULL,
  ID_PERSONA D_ID NOT NULL,
  ID_PROVINCIA D_ID NOT NULL,
  ID_MUNICIPIO D_ID NOT NULL,
  ID_DISTRITO_MUNICIPAL D_ID NOT NULL,
  ID_CODIGO_POSTAL D_ID NOT NULL,
...
(
  ...
  CORREO D_CORREO NOT NULL,
  MENSAJE D_BLOB_TEXTO NOT NULL,
  ESTADO D_BOOLEAN_T,
  CONSTRAINT INTEG_2535 PRIMARY KEY (ID)
);
...
(
  ID D_ID NOT NULL,
  USERNAME D_IDUSUARIO NOT NULL,
  ROL D_ROL NOT NULL,
  CLIENTE_SELECT D_BOOLEAN_F NOT NULL,
  CLIENTE_INSERT D_BOOLEAN_F NOT NULL,
  CLIENTE_UPDATE D_BOOLEAN_F NOT NULL,
  CLIENTE_DELETE D_BOOLEAN_F NOT NULL,
  PRODUCTO_SELECT D_BOOLEAN_F NOT NULL,
  PRODUCTO_INSERT D_BOOLEAN_F NOT NULL,
  PRODUCTO_UPDATE D_BOOLEAN_F NOT NULL,
  PRODUCTO_DELETE D_BOOLEAN_F NOT NULL,
  USUARIO_SELECT D_BOOLEAN_F NOT NULL,
  USUARIO_INSERT D_BOOLEAN_F NOT NULL,
  USUARIO_UPDATE D_BOOLEAN_F NOT NULL,
  USUARIO_DELETE D_BOOLEAN_F NOT NULL,
  CAMBIO_CLAVE D_BOOLEAN_F NOT NULL,
  FACTURA_SELECT D_BOOLEAN_F NOT NULL,
  FACTURA_INSERT D_BOOLEAN_F NOT NULL,
  FACTURA_UPDATE D_BOOLEAN_F NOT NULL,
  FACTURA_DELETE D_BOOLEAN_F NOT NULL,
  REPORTES_SELECT D_BOOLEAN_F NOT NULL,
  INVENTARIOS_SELECT D_BOOLEAN_F NOT NULL,
  TURNO_SELECT D_BOOLEAN_F NOT NULL,
  TURNO_INSERT D_BOOLEAN_F NOT NULL,
  TURNO_UPDATE D_BOOLEAN_F NOT NULL,
  TURNO_DELETE D_BOOLEAN_F NOT NULL,
  DEUDAS_SELECT D_BOOLEAN_F NOT NULL,
  DEUDAS_INSERT D_BOOLEAN_F NOT NULL,
  DEUDAS_UPDATE D_BOOLEAN_F NOT NULL,
  DEUDAS_DELETE D_BOOLEAN_F NOT NULL,
  CONSTRAINT INTEG_1094 PRIMARY KEY (ID)
);
...
INNER JOIN V_PERSONAS r ON r.ID = c.ID
INNER JOIN V_GENERALES g ON g.ID_PERSONA = c.ID;
CREATE VIEW GET_CLIENTES_SB (ID, CEDULA, PNOMBRE, SNOMBRE, APELLIDOS, SEXO, 
     DIRECCION, FECHA_NACIMIENTO, FECHA_INGRESO, ESTADO)
AS SELECT r.ID, g.CEDULA, r.PNOMBRE, r.SNOMBRE, r.APELLIDOS, r.SEXO, d.DIRECCION, 
          r.FECHA_NACIMIENTO, r.FECHA_INGRESO, r.ESTADO
FROM V_CLIENTES c 
INNER JOIN V_PERSONAS r ON r.ID = c.ID
INNER JOIN V_DIRECCIONES d ON d.ID_PERSONA = c.ID
INNER JOIN V_GENERALES g ON g.ID_PERSONA = c.ID;
CREATE VIEW GET_CLIENTES_SB_TABLA (ID, CEDULA, PERSONA, PNOMBRE, SNOMBRE, 
     APELLIDOS, FECHA_INGRESO, ESTADO)
AS SELECT r.ID, g.CEDULA, r.PERSONA, r.PNOMBRE, r.SNOMBRE, r.APELLIDOS, r.FECHA_INGRESO, r.ESTADO
FROM V_CLIENTES c
INNER JOIN V_PERSONAS r ON r.ID = c.ID
INNER JOIN V_GENERALES g ON g.ID_PERSONA = c.ID;
...
CREATE VIEW GET_TEMPORALES (ID_FACTURA, NOMBRETEMP, PNOMBRE, SNOMBRE, 
     APELLIDOS, FECHA, IDUSUARIO, HORA, ID_TURNO, EFECTIVO, CAMBIO, 
     ESTADO_FACTURA, MONTO, ID_CLIENTE)
AS SELECT f.ID, f.NOMBRETEMP, p.PNOMBRE, p.SNOMBRE, p.APELLIDOS, f.FECHA, 
...
GROUP by f.ID, f.NOMBRETEMP, p.PNOMBRE, p.SNOMBRE, p.APELLIDOS, f.FECHA, 
...
CREATE VIEW V_CONTACTS_EMAIL (ID, ID_PERSONA, EMAIL)
AS SELECT r.ID, r.ID_PERSONA, r.EMAIL
FROM CONTACTOS_EMAIL r;
CREATE VIEW V_CONTACTS_TEL (ID, ID_PERSONA, TELEFONO)
AS SELECT r.ID, r.ID_PERSONA, r.TELEFONO
...
AS SELECT r.ID, r.ID_PERSONA, r.ID_PROVINCIA, r.ID_MUNICIPIO,
     r.ID_DISTRITO_MUNICIPAL, r.ID_CODIGO_POSTAL, r.DIRECCION
FROM DIRECCIONES r;
CREATE VIEW V_DISTRITOSMUNICIPALES (ID, NOMBRE, IDMUNICIPIO)
...
CREATE VIEW V_FACTURAS (ID, ID_CLIENTE, ID_TURNO, EFECTIVO, CAMBIO, FECHA, 
     HORA, ESTADO_FACTURA, IDUSUARIO, NOMBRETEMP)
AS SELECT r.ID, r.ID_CLIENTE, r.IDTURNO, r.EFECTIVO, r.CAMBIO, r.FECHA, r.HORA, r.ESTADO_FACTURA, r.IDUSUARIO, r."nombreTemp"
FROM FACTURAS r;
...
FROM MEDICAMENTOS r;
CREATE VIEW V_MENSAJE (ID, ID_DOCTOR, ID_PACIENTE, HORA, FECHA, NOMBRE, CORREO, 
     MENSAJE, ESTADO)
AS SELECT r.ID, r.ID_DOCTOR, r.ID_PACIENTE, r.HORA, r.FECHA, r.NOMBRE, r.CORREO,
     r.MENSAJE, r.ESTADO
FROM MENSAJE r;
...
CREATE VIEW V_PERFILES (ID, USERNAME, ROL, CLIENTE_SELECT, CLIENTE_INSERT, 
     ...
     INVENTARIOS_SELECT, TURNO_SELECT, TURNO_INSERT, TURNO_UPDATE, 
     TURNO_DELETE, DEUDAS_SELECT, DEUDAS_INSERT, DEUDAS_UPDATE, DEUDAS_DELETE)
AS SELECT r.ID, r.USERNAME, r.ROL, r.CLIENTE_SELECT, r.CLIENTE_INSERT, r.CLIENTE_UPDATE,
     r.CLIENTE_DELETE, r.PRODUCTO_SELECT, r.PRODUCTO_INSERT, r.PRODUCTO_UPDATE,
     r.PRODUCTO_DELETE, r.USUARIO_SELECT, r.USUARIO_INSERT, r.USUARIO_UPDATE,
     r.USUARIO_DELETE, r.CAMBIO_CLAVE, r.FACTURA_SELECT, r.FACTURA_INSERT,
     r.FACTURA_UPDATE, r.FACTURA_DELETE, r.REPORTES_SELECT,
...
SET TERM ^ ;
CREATE TRIGGER ARS_BI FOR ARS ACTIVE
...
BEGIN
  IF (NEW.ID IS NULL) THEN
    NEW.ID = GEN_ID(G_ID_ARS, 1);
  ELSE
  BEGIN
    tmp = GEN_ID(G_ID_ARS, 0);
    if (tmp < new.ID) then
      tmp = GEN_ID(G_ID_ARS, new.ID-tmp);
  END
END
^
SET TERM ; ^
SET TERM ^ ;
CREATE TRIGGER ASEGURADOS_BI FOR ASEGURADOS ACTIVE
BEFORE INSERT POSITION 0
...
BEGIN
  IF (NEW.ID IS NULL) THEN
    NEW.ID = GEN_ID(GEN_ID_ASEGURADOS, 1);
  ELSE
  BEGIN
    tmp = GEN_ID(GEN_ID_ASEGURADOS, 0);
    if (tmp < new.ID) then
      tmp = GEN_ID(GEN_ID_ASEGURADOS, new.ID-tmp);
  END
END
^
SET TERM ; ^
SET TERM ^ ;
CREATE TRIGGER CATEGORIAS_BI FOR CATEGORIAS ACTIVE
BEFORE INSERT POSITION 0
...
BEGIN
  IF (NEW.ID IS NULL) THEN
    NEW.ID = GEN_ID(G_ID_CATEGORIAS, 1);
  ELSE
  BEGIN
    tmp = GEN_ID(G_ID_CATEGORIAS, 0);
    if (tmp < new.ID) then
      tmp = GEN_ID(G_ID_CATEGORIAS, new.ID-tmp);
  END
END
^
SET TERM ; ^
...
SET TERM ; ^
SET TERM ^ ;
CREATE TRIGGER PRODUCTOS_BI FOR PRODUCTOS ACTIVE
BEFORE INSERT POSITION 0
...
BEGIN
  IF (NEW.ID IS NULL) THEN
    NEW.ID = GEN_ID(G_ID_PRODUCTOS, 1);
  ELSE
  BEGIN
    tmp = GEN_ID(G_ID_PRODUCTOS, 0);
    if (tmp < new.ID) then
      tmp = GEN_ID(G_ID_PRODUCTOS, new.ID-tmp);
  END
END
^
SET TERM ; ^
SET TERM ^ ;
CREATE TRIGGER RECCOUNT_BI FOR RECCOUNT INACTIVE
BEFORE INSERT POSITION 0
...
SET TERM ; ^
SET TERM ^ ;
CREATE TRIGGER RECCOUNT_BI_2 FOR RECCOUNT ACTIVE
BEFORE INSERT POSITION 0
...
SET TERM ^ ;
ALTER PROCEDURE SP_ACTUALIZAR_CANTIDAD_FILAS (
    TNCODSUC D_ID )
SQL SECURITY DEFINER
...
BEGIN
     ...
     VALUES (
          ...
          :i_APELLIDOS, --Si
          :i_SEXO, --Si
          :i_FECHA_NACIMIENTO, --Si
          :i_ESTADO --Si
          
     );
     
     INSERT INTO V_GENERALES (ID_PERSONA, CEDULA, ESTADO_CIVIL) 
     VALUES(
          :V_ID,
          :i_CEDULA, --Si
          :i_ESTADO_CIVIL--Si
     ...
     INSERT INTO V_DIRECCIONES (ID_PERSONA, ID_PROVINCIA, ID_MUNICIPIO,
          ID_DISTRITO_MUNICIPAL, DIRECCION)
     VALUES (
          :V_ID,
          :i_IDPROVINCIA, --Si 
          :i_IDMUNICIPIO, --Si
          :i_IDDISTRITOMUNICIPAL, --Si
          :i_DIRECCION --Si
     );
     ...
     EXECUTE PROCEDURE SP_RECCOUNT_ESTADO('I', 'GET_CLIENTES_ESTADO_SB', :I_ESTADO);
...
SET TERM ^ ;
ALTER PROCEDURE SP_UPDATE_CLIENTE (
    I_ID D_ID,
    I_IDPROVINCIA D_ID,
    I_IDMUNICIPIO D_ID,
    I_IDDISTRITOMUNICIPAL D_ID,
...
BEGIN
     ...
     SET 
          a.ID_PROVINCIA = :I_IDPROVINCIA, 
          a.ID_MUNICIPIO = :I_IDMUNICIPIO, 
          a.ID_DISTRITO_MUNICIPAL = :I_IDDISTRITOMUNICIPAL, 
...
n = nula
t = temporal';
COMMENT ON COLUMN FACTURAS."nombreTemp" IS 'Nombre utilizado de manera temporal de la factura, para indentificar el cliente.';
...
ALTER TABLE MENSAJE ADD CONSTRAINT MENSAJE_FK
  FOREIGN KEY (ID_PACIENTE) REFERENCES PACIENTES (ID);
...
GRANT EXECUTE
 ON PROCEDURE SP_ACTUALIZAR_CANTIDAD_FILAS TO  SYSDBA WITH GRANT OPTION;
...
GRANT EXECUTE
 ON PROCEDURE SP_UPDATE_CLIENTE TO  SYSDBA WITH GRANT OPTION;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON DETALLE_FACTURA TO  SYSDBA WITH GRANT OPTION;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON INSCRIPCION TO  SYSDBA WITH GRANT OPTION;

GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON MEDICAMENTOS TO  SYSDBA WITH GRANT OPTION;

GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON MENSAJE TO  SYSDBA WITH GRANT OPTION;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON GET_CLIENTES_SB_TABLA TO  SYSDBA WITH GRANT OPTION;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON V_DISTRITOSMUNICIPALES TO  SYSDBA WITH GRANT OPTION;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON V_MENSAJE TO  SYSDBA WITH GRANT OPTION;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON V_PERFILES TO  SYSDBA WITH GRANT OPTION;
